...
import { useGetTarefas } from "../../api/tarefas";
...
function ReactTable({
  ...
  const groupedDataByStatus = data.reduce((acc, curr) => {
    const statusName = curr.status && curr.status.length > 0 ? curr.status[0].nome : 'Sem Status';
    if (!acc[statusName]) acc[statusName] = [];
    acc[statusName].push(curr);
    return acc;
  }, {});

  const statusTabs = Object.keys(groupedDataByStatus);

  // Atualizar o filtro de status ao clicar em um chip
  const handleTabChange = (event, newValue) => {
    setSelectedStatus(newValue);
    if (newValue !== 'Todos') {
      const filteredData = groupedDataByStatus[newValue];
      setSortedData(filteredData); // Armazenar os dados filtrados
    } else {
      setSortedData(data); // Mostrar todos os dados se a aba 'Todos' for selecionada
    }
  };
  ...
  return (
    ...
    <Box mb={2}>
        ...
        >
          ...
          {statusTabs.map((status) => (
            <Tab
              ...
              label={
                <Chip
                  label={`${status} (${groupedDataByStatus[status].length})`}
                  clickable
                />
              }
              value={status}
      ...
      >
        ...
        >
          <Button
            onClick={() => setIsModalDocumentosOpen(true)}
            variant="contained"
...
const ListagemTarefas = (andamentoIdDetalhes) => {
  const theme = useTheme();
  const isDarkMode = theme.palette.mode === "dark";
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const { tarefas: lists, isLoading } = useGetTarefas(
    formData,
  ...
  const TagsCell = ({ row }) => {
    ...
    return (
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        />
        {status.length > 1 && (
          <Tooltip title={status.slice(1).map(statu => statu.nome).join(', ')} placement="top">
  ...
  const columns = useMemo(
    () => [
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            <Box
              sx={{
                ...
                height: 16,
                borderRadius: '50%',
                backgroundColor: row.original.prioridade,
              }}
            />
          </Box>
        ),
      ...
      {
        header: "Nome",
        accessorKey: "nomeTarefa",
      },
      {
        header: "Tipo de tarefa",
        accessorKey: "tipoTarefa",
      },
      {
        header: "Prazo",
        accessorKey: "prazo",
        ...
        sortingFn: (rowA, rowB) => {
          const dateA = new Date(
            rowA.original.prazo.split("/").reverse().join("-")
          ).getTime();
          const dateB = new Date(
            rowB.original.prazo.split("/").reverse().join("-")
          ).getTime();
          return dateA - dateB;
        },
        cell: ({ row }) => (
      ...
      },
      {
        ...
        accessorKey: 'status',
        enableGrouping: false,
        accessorFn: row => (Array.isArray(row.status) && row.status.length ? row.status.map(statu => statu.nome).join(', ') : 'Nenhuma'),
        cell: TagsCell,
      },
      {
        header: "Responsável",
...
const ModalAdicionarDocumentos = ({ open, handleClose }) => {
  ...
  const [novoDocumento, setNovoDocumento] = useState({ nome: "", tipoDocumento: "", descricao: "", tags: [] });

  // Função para lidar com o drop de arquivos
  const onDrop = (acceptedFiles) => {
    const novosDocumentos = acceptedFiles.map((file) => ({
      nome: file.name,
      tipoDocumento: "",
      descricao: "",
      tags: []
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="lg">
          ...
          <Grid item xs={12}>
            ...
            >
              ...
              {isDragActive ? (
                ...
                <p>Arraste e solte arquivos aqui, ou clique para selecionar os arquivos</p>
              )}
            </Box>
          </Grid>

          {/* Tabela de Tarefas Adicionados */}
          <Grid item xs={12}>
            <TableContainer>
              <Table>
                <TableHead>
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        <Autocomplete
                          ...
                          onChange={(_, newValue) => {
                            ...
                            updatedDocumentos[index].tipoDocumento = newValue;
                      ...
                      <TableCell>
                        <Autocomplete
                          ...
                          onChange={(_, newValue) => {
                            const updatedDocumentos = [...documentos];
                            updatedDocumentos[index].tags = newValue;
                            setDocumentos(updatedDocumentos);
                          }}
                          renderInput={(params) => <TextField {...params} />}
                        />
      ...
      <DialogActions>
        ...
        <Button
          onClick={() => {
            setNovoDocumento({ nome: "", tipoDocumento: "", descricao: "", tags: [] });
          }}
          startIcon={<AddIcon />}
        >
          Adicionar Documento
