...
import { useGetDocumentos } from "../../api/documentos";
...
// Função para baixar um arquivo específico via Base64
...
// Função para baixar documentos selecionados como um ZIP
const downloadSelectedDocumentsAsZip = async (selectedRows) => {
  const zip = new JSZip();
  
  selectedRows.forEach((row) => {
    const { nomeDocumento, base64Content } = row.original; // Supondo que o conteúdo do documento está em 'base64Content'
    zip.file(nomeDocumento, base64Content, { base64: true });
  });

  const zipContent = await zip.generateAsync({ type: 'blob' });
  saveAs(zipContent, 'documentos_selecionados.zip');
};
...
function ActionCell({ row }) {
  ...
  const handleDownload = () => {
    const { nomeDocumento, base64Content } = row.original;
    downloadBase64File(base64Content, nomeDocumento);
  };

  const handleDownloadSelected = (selectedRows) => {
    if (selectedRows.length > 0) {
      downloadSelectedDocumentsAsZip(selectedRows);
    }
  };
...
const ListagemDocumentos = (andamentoIdDetalhes) => {
  const theme = useTheme();
  const isDarkMode = theme.palette.mode === "dark";
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const { documentos: lists, isLoading } = useGetDocumentos(
    formData,
      ...
      },
      {
        ...
        sortingFn: (rowA, rowB) => {
          const dateA = new Date(
            rowA.original.dataCadastro.split("/").reverse().join("-")
          ).getTime();
          const dateB = new Date(
            rowB.original.dataCadastro.split("/").reverse().join("-")
          ).getTime();
          return dateA - dateB;
        },
        cell: ({ row }) => (
          <div>
            {isFiltered
              ? formatDate(row.original.dataCadastro)
              : row.original.dataCadastro}
          </div>
        ),
      },
