...
import { useGetDocumentos } from "../../api/causaRaizMotivo";
      ...
      >
        <Stack direction="row" spacing={1} alignItems="center">
          <DebouncedInput
            ...
            placeholder="Busque pelo motivo ou sub-motivo"
            style={{
              width: "550px",
              height: "33px",
              borderRadius: "8px",
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const tags = row.original.subMotivo || [];
      
          // Remove duplicadas com base no nome ou ID
          const uniqueTags = Array.from(new Map(tags.map((tag) => [tag.nome, tag])).values());
      
          const maxTagsToShow = 2;
          const remainingTagsCount = uniqueTags.length - maxTagsToShow;
      
          return (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
              {uniqueTags.slice(0, maxTagsToShow).map((tag) => (
                <Chip
                  key={tag.id}
                  label={tag.nome}
                  sx={{
                    backgroundColor: tag.cor || "rgba(27, 57, 122, 0.08)", 
              ...
              {remainingTagsCount > 0 && (
                <Tooltip
                title={
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", backgroundColor: "#FFF", padding: "8px", borderRadius: "4px" }}>
                    {uniqueTags.slice(maxTagsToShow).map((tag) => (
                      <Chip
                        key={tag.id}
                        label={tag.nome}
                        sx={{
                          backgroundColor: "rgba(27, 57, 122, 0.08)", 
                          color: "rgba(33, 33, 33, 1)", 
                          fontWeight: 400,
                          fontSize: "12px",
                          height: "28px",
                ...
                arrow
                sx={{ tooltip: { backgroundColor: "#FFF", color: "rgba(0, 0, 0, 0.87)" } }}
              >
                <Chip
