...
import {
  DialogTitle, Dialog, List, FormGroup, ListItem, MenuList, MenuItem, Popover, ListItemIcon, ListItemText, ListItemSecondaryAction, Checkbox, Radio, TextField, Autocomplete, RadioGroup, FormControlLabel, IconButton, DialogActions, DialogContent, Button, Box, Grid, Typography,
  InputLabel
} from '@mui/material';
...
import { faTriangleExclamation, faXmark, faPenToSquare, faTrashAlt } from '@fortawesome/free-solid-svg-icons';
...
function ColumnsLayouts() {
  ...
  const [novoMotivo, setNovoMotivo] = useState(''); // Novo motivo a ser adicionado
  ...
  const [formData, setFormData] = useState({
    ...
    fase: '',
    exigeDocumento: 'Não',
    encerrarComPendencia: 'Sim',
    permiteAnexoDocumento: 'Não',
    exigeProvisionamento: 'Não',
    exigeTarefa: false,
  ...
  useEffect(() => {
    setStatusRegistros((prevStatus) => {
      // Adiciona "Ativo" apenas para novos registros
      const novosStatus = Array(motivos.length - prevStatus.length).fill("Ativo");
      return [...prevStatus, ...novosStatus];
    });
  }, [motivos]);
  ...
  const handleSelectAll6 = (event, newValue) => {
    if (newValue.length > 0 && newValue[newValue.length - 1].id === 'all') {
      if (formData.tipoDocumento.length === tiposDocumentos.length) {
        ...
        // Select all
        setFormData({ ...formData, tipoDocumento: tiposDocumentos.map(tipoDocumento => tipoDocumento.id) });
      }
    } else {
      tratarMudancaInputGeral('tipoDocumento', newValue.map(item => item.id));
    }
  };
  ...
  useEffect(() => {
    if (statusDadosEditar) {
      ...
      const exigeTarefa = statusDadosEditar.exigeTarefa || false;
      const exigePagamento = statusDadosEditar.exigePagamento || false;
      const exigeDeposito = statusDadosEditar.exigeDeposito || false;
      ...
      setFormData((prev) => ({
        ...
        exigePagamento,
        exigeDeposito,
        encerrarComPendencia:
          exigeTarefa || exigePagamento || exigeDeposito ? "Não" : prev.encerrarComPendencia,
        permiteAnexoDocumento: statusDadosEditar.permiteAnexoDocumento ? "Sim" : "Não",
        exigeProvisionamento: statusDadosEditar.exigeProvisionamento ? "Sim" : "Não",
      }));
  ...
  useEffect(() => {
    fetchData(`${API_QUERY}/api/TipoDocumento/ListarCombo?ativo=true`, setTiposDocumentos);
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleRadioChange2 = (event) => {
    ...
    setFormData({
      ...formData,
      encerrarComPendencia: value,
      // Resetando os checkboxes se "Sim" for selecionado
      ...(value === 'Sim' ? { exigeTarefa: false, exigePagamento: false, exigeDeposito: false } : {})
    });
  };

  const handleRadioChange3 = (event) => {
    setFormData({ ...formData, permiteAnexoDocumento: event.target.value });
  };

  const handleRadioChange4 = (event) => {
    setFormData({ ...formData, exigeProvisionamento: event.target.value });
  };
  ...
  const allSelected6 = formData.tipoDocumento.length === tiposDocumentos.length && tiposDocumentos.length > 0;
  ...
  const tratarSubmit = async () => {
    ...
    const payload = {
      ...
      exigeTarefa: formData.exigeTarefa,
      exigePagamento: formData.exigePagamento,
      exigeDeposito: formData.exigeDeposito,
      exigeProvisionamento: formData.exigeProvisionamento === "Sim",
      permiteAnexoDocumento: formData.permiteAnexoDocumento === "Sim",
      motivoStatus,
      tipoDocumentoStatus,
    };
    ...
    try {
      ...
      enqueueSnackbar(`Status de Processo (Encerrado) editado com sucesso.`, {
        variant: "success",
        anchorOrigin: { vertical: "top", horizontal: "center" },
      });
  ...
  return (
    <>
      ...
      <Grid container spacing={1}>
        <Grid item xs={12}>
          <Typography variant="body1" fontWeight={600} fontSize={'13px'} gutterBottom>
            Permite encerrar processo com pendências em aberto?
          </Typography>
          <RadioGroup
            row
            value={formData.encerrarComPendencia}
        ...
        {formData.encerrarComPendencia === 'Não' && (
          <Grid item xs={12}>
            <Box
              sx={{
                width: '856px',
            ...
            >
              <Typography
                ...
                fontWeight={600}
              >
                Escolha os tipos de pendências:
              </Typography>
              <FormGroup>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={formData.exigeTarefa}
                      onChange={(e) => setFormData({ ...formData, exigeTarefa: e.target.checked })}
                    />
                  }
                  label="Tarefas pendentes ou atrasadas"
                />
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={formData.exigePagamento}
                      onChange={(e) => setFormData({ ...formData, exigePagamento: e.target.checked })}
                    />
                  }
                  label="Pagamentos pendentes ou atrasados"
                />
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={formData.exigeDeposito}
                      onChange={(e) => setFormData({ ...formData, exigeDeposito: e.target.checked })}
                    />
                  }
                  label="Depósito pendente de baixa"
                />
              </FormGroup>
        ...
        <Grid item xs={12} mt={3}>
          <Typography variant="body1" fontWeight={600} fontSize={'13px'} gutterBottom>
            Permite a anexação de documento para encerrar um processo?
          </Typography>
          <RadioGroup
            row
            value={formData.permiteAnexoDocumento}
        ...
        {formData.exigeDocumento === 'Sim' && (
          <Grid item xs={12}>
            <Box sx={{ width: '856px', height: '154px', border: '0.6px solid rgba(171, 190, 209, 1)', borderRadius: '8px', backgroundColor: 'rgba(238, 245, 252, 1)', padding: '16px' }}>
              <Typography marginTop={1} marginBottom={2} variant="body1" color="rgba(28, 82, 151, 1)" fontSize="12px" fontWeight={600}>
                Selecione os tipos de documentos que poderão ser anexados ao encerramento do processo
              </Typography>

              <InputLabel style={{ fontSize: '12px', fontWeight: 400, marginBottom: 8 }}>Tipo de documento *</InputLabel>
              <Autocomplete
                multiple
                disableCloseOnSelect
                options={[{ id: "all", nome: "Selecionar todos" }, ...tiposDocumentos]}
                getOptionLabel={(option) => option.nome}
                value={formData.tipoDocumento.map((id) => tiposDocumentos.find((tipoDocumento) => tipoDocumento.id === id) || id)}
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    placeholder={formData.tipoDocumento.length > 0 ? "" : "Digite ou selecione um tipo de documento"}
                    error={(formData.tipoDocumento.length === 0 || formData.tipoDocumento.every((val) => val === 0)) && formValidation.tipoDocumento === false}
        ...
        <Grid item xs={12} mt={3}>
          <Typography variant="body1" fontWeight={600} fontSize={'13px'} gutterBottom>
            O provisionamento precisa ser alterado?
          </Typography>
          <RadioGroup
            row
            value={formData.exigeProvisionamento}
        ...
        <Grid item xs={9} mt={5}>
          ...
          >
            ...
            <Typography variant="body1" color="rgba(77, 77, 77, 1)" fontWeight={600} gutterBottom marginBottom={5}>
              Adicione motivos para justificar o porque você está ativando o processo.
            </Typography>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              />
              <Button variant="contained" color="primary" onClick={handleAdicionarMotivo}>
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography component="div" style={{ marginTop: '20px' }}>
            Se este motivo ficar inativo, não será mais possível cadastrar novos processos nele. Tem certeza de que deseja inativar?
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography component="div" style={{ marginTop: '20px' }}>
            Caso precise deste motivo no futuro, será necessário criar um novo. Tem certeza de que deseja continuar?
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      {requisicao === 'Editar' && (
        <AndamentoHistoricoDrawer row={statusDadosEditar} open={drawerOpen} onClose={handleDrawerClose} vindoDe={'Tipo de Andamentos'} />
      )}
    </>
  );
}
