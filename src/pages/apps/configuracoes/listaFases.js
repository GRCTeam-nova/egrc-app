...
import { useGetOrgao } from "../../../api/fases";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-fase`, {
                    state: { indoPara: "NovaFase" },
                  });
                }}
...
const toggleStatus = async () => {
  ...
  try {
    ...
    if (newStatus === "Inativo") {
      ...
      const putPayload = {
        ...
        faseUnidade: [
          {
            id: 0,
            unidadeId: unidades[0].unidadeId,
          },
        ],
      };

      response = await fetch(`${API_COMMAND}/api/Fase/Editar`, {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      setStatus(newStatus);
      const message = `Fase ${row.original.nome} ${newStatus.toLowerCase()}.`;
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${API_COMMAND}/api/Fase/${row.original.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        enqueueSnackbar(`Fase ${row.original.nome} excluída.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir o fase: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a fase "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa fase não aparecerá mais no cadastro manual do
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir a fase "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Essa fase não será mais disponibilizada ao cadastrar um novo
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Não é possível excluir a fase.
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            A fase não pode ser excluída pois está vinculada a processos. É
            possível inativar a fase nas configurações de edição.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemFase = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetOrgao(formData, processoSelecionadoId);
