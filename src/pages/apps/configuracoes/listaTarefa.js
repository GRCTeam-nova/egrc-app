...
import { useGetAcaoJudicial } from "../../../api/acoesJudiciais";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-tarefa`, {
                    state: { indoPara: "NovaTarefa" },
                  });
                }}
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus(newStatus);
        if (newStatus === "Inativo") {
          enqueueSnackbar(`Ação judicial ${row.original.nome} inativada.`, {
            variant: "success",
          ...
          enqueueSnackbar(`Ação judicial ${row.original.nome} ativada.`, {
            variant: "success",
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: "top",
      ...
      >
        <Stack>
          <Button
            ...
            onClick={() => {
              const AcaoJudicialDados = row.original;
              navigation(`NovaTarefa/apps/processos/nova-tarefa`, {
                state: {
...
const ListagemAcoesJudiciais = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetAcaoJudicial(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshAcoes} />
        ), // Passa refreshData
      },
    ],
    [theme]
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshAcoes, // Passa refreshData para ReactTable
              }}
            />
          )}
        </Box>
...
export default ListagemAcoesJudiciais;
