...
import { useGetTipoAudiencia } from "../../../api/tipoAudiencia";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/novo-tipo-audiencia`, {
                    state: { indoPara: "NovoTipoAudiencia" },
                  });
                }}
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus(newStatus);
        const message = `Tipo de audiência ${
          row.original.nome
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const TipoAudienciaDados = row.original;
              navigation(`/apps/processos/novo-tipo-audiencia`, {
                state: {
...
const ListagemTipoDeAudiencias = () => {
  const theme = useTheme();
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const {
    tipoAudiencias: lists,
    isLoading,
    refetch,
  } = useGetTipoAudiencia(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshTipoAudiencias} />
        ),
      },
    ],
    [theme]
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshTipoAudiencias,
              }}
            />
          )}
        </Box>
...
export default ListagemTipoDeAudiencias;
