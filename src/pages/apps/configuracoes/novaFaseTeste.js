...
function ColumnsLayouts() {
      ...
      const fetchEmpresaDados = async () => {
        try {
          const response = await fetch(
            `https://api.egrc.homologacao.com.br/api/v1/projects/tests/phases/${dadosApi.idTestPhase}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleConfirmSample = async () => {
    ...
    try {
      await axios.put(
        "https://api.egrc.homologacao.com.br/api/v1/projects/tests/phases/attributes/result/sample",
        {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (requisicao === "Editar") {
        ...
        url = `https://api.egrc.homologacao.com.br/api/v1/projects/tests/${TesteId}/phases/${faseTesteDados?.idTestPhase}`;
        method = "PUT";
        payload = {
          idTestPhase: faseTesteDados?.idTestPhase,
          name: nomeFaseTeste,
  ...
  const handleRetornar = async () => {
    ...
    try {
      setLoading(true);
      const url = `https://api.egrc.homologacao.com.br/api/v1/projects/tests/${TesteId}/phases/${faseTesteDados.idTestPhase}`;
      const payload = {
        idTestPhase: faseTesteDados.idTestPhase,
        name: nomeFaseTeste,
        description: descricao,
  ...
  const tratarSubmit = async () => {
    ...
    try {
      ...
      if (requisicao === "Criar") {
        url = `https://api.egrc.homologacao.com.br/api/v1/projects/tests/${TesteId}/phases`;
        method = "POST";
        payload = {
          name: nomeFaseTeste,
          population: Number(populacao),
