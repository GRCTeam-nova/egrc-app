...
function ColumnsLayoutsDrawer({ buttonSx, onNormativeCreated }) {
  ...
  const tratarSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Não foi possível cadastrar a normativa.");
      }
      ...
      if (onNormativeCreated) {
        const novaNormativa = {
          id: data.data.idNormative,
          nome: nome,
        };
        onNormativeCreated(novaNormativa);
      }
    } catch (error) {
      console.error(error.message);
      enqueueSnackbar("Erro ao cadastrar a normativa.", { variant: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Tooltip title="Cadastre uma normativa" arrow placement="top">
        <Button
  variant="contained"
  size="small"
  onClick={(event) => {
