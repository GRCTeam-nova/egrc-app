...
import { useGetRotina } from "../../../api/rotina";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      const isAtivo = item.ativo === 'Ativo' ? true : false;
  
      if (isActiveFilter && isAtivo) return true;
      if (isInactiveFilter && !isAtivo) return true;
      return false;
    });
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-rotina`, {
                    state: { indoPara: "NovaRotina" },
                  });
                }}
...
const ListagemTiposAndamentos = () => {
  ...
  const {
    statusProcessos: lists,
    isLoading,
    refetch,
  } = useGetRotina(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div>
            {row.original.unidades.map((unidade, index) => (
      ...
      
      {
        ...
        cell: ({ row }) => {
          const responsaveis = row.original.responsaveis;
          const maxToShow = 2; // Limite de chips exibidos
  
          return (
            <>
              {responsaveis.slice(0, maxToShow).map((responsavel, index) => (
                <Chip
                  key={index}
                  label={responsavel}
                  sx={{
                    ...
                    backgroundColor: "rgba(219, 237, 255, 1)"
                  }}
                />
              ))}
              {responsaveis.length > maxToShow && (
                <Tooltip
                  title={responsaveis.slice(maxToShow).join(", ")}
                  arrow
                >
                  <Chip
                    label={`+${responsaveis.length - maxToShow}`}
                    sx={{
                      marginRight: "4px",
                      marginBottom: "4px",
                      fontSize: "12px",
