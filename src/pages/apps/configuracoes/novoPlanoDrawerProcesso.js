...
function ColumnsLayoutsDrawer({ buttonSx, onPlansCreated }) {
  ...
  const [nomePlano, setNome] = useState("");
  ...
  useEffect(() => {
    if (open) {
      setNome("");
      setCodigo("");
      setFormValidation({ nomePlano: true, codigo: true, prioridade: true });
      setHasChanges(false);
    }
  }, [open]);
  ...
  const tratarSubmit = async () => {
    ...
    const missingFields = [];
    if (!nomePlano.trim()) {
      setFormValidation((prev) => ({ ...prev, nomePlano: false }));
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Não foi possível cadastrar o plano de ação.");
      }

      const data = await response.json();

      enqueueSnackbar(`Plano de ação ${mensagemFeedback} com sucesso!`, {
        variant: "success",
      });
      setOpen(false);
      ...
      if (onPlansCreated) {
        const novoPlano = {
          id: data.data.idActionPlan,
          nome: nomePlano,
        };
        onPlansCreated(novoPlano);
      }
    } catch (error) {
      console.error(error.message);
      enqueueSnackbar("Erro ao cadastrar o plano de ação.", {
        variant: "error",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <Tooltip title="Cadastre um plano" arrow placement="top">
        <Button
          variant="contained"
          size="small"
          onClick={(event) => {
              ...
              <Grid item xs={12} mb={3}>
                <Stack spacing={1}>
                  ...
                  <TextField
                    ...
                    fullWidth
                    value={nomePlano}
                    error={!nomePlano && !formValidation.nomePlano}
                  />
                </Stack>
              </Grid>
