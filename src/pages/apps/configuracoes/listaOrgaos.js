...
import { useGetOrgao } from "../../../api/orgaos";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/novo-orgao`, {
                    state: { indoPara: "NovoOrgao" },
                  });
                }}
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        ...
        const areas = row.original.area
          ? row.original.area.map((area) => ({
              id: 0,
              areaId: area.unidadeId,
            }))
          : [];

        const putPayload = {
          id: row.original.id,
          orgaoUnidade: unidades,
          areaOrgao: areas,
        ...
        response = await fetch(`${API_COMMAND}/api/Orgao/Editar`, {
          method: "POST",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        enqueueSnackbar(`Órgão ${row.original.nome} excluído.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir o órgão: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o órgão "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, esse órgão não aparecerá mais no cadastro manual do
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir o órgão "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Esse órgão não será mais disponibilizado ao cadastrar um novo
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Não é possível excluir o Órgão.
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            O órgão não pode ser excluído pois está vinculado a processos. É
