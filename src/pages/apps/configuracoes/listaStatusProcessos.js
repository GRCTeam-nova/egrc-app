...
import { useGetStatusProcessos } from "../../../api/statusProcessos";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const recordType = "Status do Processos";
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          throw new Error(
            `Falha ao enviar o formulário: ${response.status} ${response.statusText
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const statusDadosEditar = row.original;
              const rota =
                row.original.nome === "Encerrado"
                  ? "/apps/processos/editar-status-encerrado"
                  : row.original.nome === "Suspenso"
                    ? "/apps/processos/editar-status-suspenso"
                    : "/apps/processos/editar-status-ativo";
              navigation(rota, {
                state: {
                  indoPara: "StatusProcesso",
                  statusDadosEditar,
                },
              });
              handleClose();
...
const ListagemTiposAndamentos = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetStatusProcessos(formData, processoSelecionadoId);
