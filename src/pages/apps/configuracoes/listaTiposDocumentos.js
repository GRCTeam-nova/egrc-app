...
import { useGetTiposDocumentos } from "../../../api/TiposDocumentos";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const recordType = "Tipos Documentos";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/novo-tipo-documento`, {
                    state: { indoPara: "NovaTipoDocumento" },
                  });
                }}
                            ...
                            >
                              {header.isPlaceholder ? null : (
                                ...
                                >
                                  ...
                                  {header.id === "ativo" && (
                                    <Tooltip title="Mostrar/Esconder Ativos e Inativos" placement="top">
...
>
    ...
    <Box p={2} sx={{ width: '200px' }}>  {/* Aumenta o tamanho da caixa */}
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        ...
        response = await fetch(
          `${API_COMMAND}/api/TiposDocumento/Editar`,
          {
            method: "PUT",
            headers: {
        ...
        // Se o novo status for Ativo, faz a atualização
        const unidades = row.original.unidade.map((unidade) => ({
          id: 0,
          acao: "null",
          acaoId: row.original.id,
      ...
      if (response.ok) {
        setStatus(newStatus);
        if (newStatus === "Inativo") {
          enqueueSnackbar(`NovoTipoDocumento ${row.original.nome} inativada.`, {
            variant: "success",
          ...
          enqueueSnackbar(`NovoTipoDocumento ${row.original.nome} ativada.`, {
            variant: "success",
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: "top",
        ...
        try {
          ...
          throw new Error(
            `Falha ao enviar o formulário: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`
          );
        } catch {
          throw new Error(
    ...
    >
      ...
      >
        <Stack>
          <Button
            ...
            onClick={() => {
              const DocumentosDados = row.original;
              navigation(`/apps/processos/novo-tipo-documento`, {
                state: {
                  indoPara: "NovoTipoDocumento",
                  DocumentosDados,
                },
              });
              handleClose();
          ...
          <Button
            ...
            color={row.original.ativo === true ? "error" : "success"}
          // startIcon={
          //   status === "Ativo" ? (
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a ação "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa ação não aparecerá mais no cadastro
            manual.
          </Typography>
        </DialogContent>
        <DialogActions>
...
const ListagemTipoDocementos = () => {
  const theme = useTheme();
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const {
    tiposDocumentos: lists,
    isLoading,
    refetch,
  } = useGetTiposDocumentos(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
...

      {
        ...
        cell: ({ cell }) => {
          const formattedDate = new Date(cell.getValue()).toLocaleDateString('pt-BR', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          });
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshDocumentos} />
        ), // Passa refreshData
      },
    ],
    [theme]
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshDocumentos, // Passa refreshData para ReactTable
              }}
            />
          )}
        </Box>
...
export default ListagemTipoDocementos;
