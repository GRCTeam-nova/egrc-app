...
function ColumnsLayoutsDrawer({ buttonSx, onProcessCreated }) {
  ...
  const tratarSubmit = async () => {
    ...
    if (missingFields.length > 0) {
      enqueueSnackbar(`Os campos ${missingFields.join(" e ")} são obrigatórios!`, { variant: "error" });
      return;
    }
    ...
    if (requisicao === 'Criar') {
      ...
      payload = {
        ...
        code: codigo,
        idCompanies: Array.isArray(idCompany) ? idCompany : [idCompany]
      };
    }
    ...
    try {
      ...
      enqueueSnackbar(`Processo ${mensagemFeedback} com sucesso!`, { variant: 'success' });
      setOpen(false);
  ...
  return (
    <>
      ...
      <Drawer anchor="right" open={open} onClose={() => setOpen(false)} PaperProps={{ sx: { width: 670 } }}>
      <Box sx={{ width: 650, p: 3 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
      <Box component="h2" sx={{ color: '#1C5297', fontWeight: 600, fontSize: '16px' }}>Cadastrar Processo</Box>
              ...
              <Grid item xs={12} mb={3}>
                <Stack spacing={1}>
                  ...
                  <TextField
                    ...
                    error={!nomeDepartamento && !formValidation.nomeDepartamento}
                  />
                </Stack>
              </Grid>
            </Grid>
