...
import { useGetParte } from "../../../api/partes";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const [anchorElNova, setAnchorElNova] = useState(null);

  const navigation = useNavigate();

  // Funções para o Popover de Filtro
  ...
  // Funções para o Popover "Nova"
  const handleClickNova = (event) => {
    event.stopPropagation();
    setAnchorElNova(event.currentTarget);
  };

  const handleCloseNova = () => {
    setAnchorElNova(null);
  };


  const openFilter = Boolean(anchorElFilter);
  const openNova = Boolean(anchorElNova);

  const idFilter = openFilter ? "status-filter-popover" : undefined;
  const idNova = openNova ? "nova-popover" : undefined;
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              ...
              <Popover
                id={idNova}
                open={openNova}
                anchorEl={anchorElNova}
                onClose={handleCloseNova}
                anchorOrigin={{
                  vertical: "bottom",
                  horizontal: "left",
                }}
              >
                <div style={{ padding: 10, width: "150px" }}>
                  <Button
                    variant="text"
                    onClick={() => {
                      navigation(`/apps/processos/nova-parte`, { state: { indoPara: "NovaParte" } });
                      handleCloseNova();
                    }}
                  ...
                  <Button
                    variant="text"
                    onClick={() => {
                      navigation(`/apps/processos/nova-pessoa-juridica`, { state: { indoPara: "NovaPessoaJuridica" } });
                      handleCloseNova();
                    }}
                  ...
                  <Button
                    variant="text"
                    onClick={() => {
                      navigation(`/apps/processos/novo-advogado`, { state: { indoPara: "NovoAdvogado" } });
                      handleCloseNova();
                    }}
                    startIcon={<WorkOutlinedIcon sx={{ fontSize: 20 }} />}
                    sx={{
                      justifyContent: "flex-start",
                      textAlign: "left",
                      fontSize: "13px",
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        ...
        response = await fetch(`${API_COMMAND}/api/TipoAudiencia/Editar`, {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        setStatus(newStatus);
        const message = `Tipo de audiência ${row.original.nome
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${API_COMMAND}/api/TipoAudiencia/${row.original.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        enqueueSnackbar(`Tipo de audiência ${row.original.nome} excluído.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir o tipo de audiência: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const ParteDados = row.original;
              navigation(`/apps/processos/nova-parte`, {
                state: {
          ...
          <Button
            onClick={() => {
              if (row.original.ativo === true) handleDialogOpen();
              else toggleStatus();
            }}
            style={{ color: "#707070", fontWeight: 400 }}
          >
            {row.original.ativo === true ? "Inativar" : "Ativar"}
          </Button>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o tipo de audiência "
            {row.original.nome}"?
          </Typography>
          <Typography
            component="div"
          ...
          >
            Ao inativar, esse tipo de audiência não aparecerá mais no cadastro
            manual.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir o tipo de audiência "
            {row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Esse tipo de audiência não será mais disponibilizado ao cadastrar um
            novo processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Não é possível excluir esse tipo de audiência.
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            O tipo de audiência não pode ser excluído pois está vinculado a
            processos. É possível inativar o tipo de audiência nas configurações
            de edição.
          </Typography>
        </DialogContent>
        <DialogActions>
...
const ListagemPartes = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetParte(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ cell }) => {
          const formattedDate = new Date(cell.getValue()).toLocaleDateString(
            "pt-BR",
            {
              day: "2-digit",
              month: "2-digit",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Chip
            label={row.original.ativo === true ? "Ativo" : "Inativo"}
            color={row.original.ativo === true ? "success" : "error"}
            sx={{
              backgroundColor: "transparent",
              color: "#00000099",
              fontWeight: 600,
              ...
              "& .MuiChip-icon": {
                color:
                  row.original.ativo === true ? "success.main" : "error.main",
                marginLeft: "4px",
              },
            }}
            icon={
              <span
                style={{
                  backgroundColor:
                    row.original.ativo === true ? "green" : "red",
                  borderRadius: "50%",
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshPartes} />
        ),
      },
    ],
    [theme]
