...
function ColumnsLayoutsDrawer({ buttonSx, onDeficienciaCreated }) {
  const [open, setOpen] = useState(false);
  const { token } = useToken();
  const location = useLocation();
  const { dadosApi } = location.state || {};
  const [nomeDeficiencia, setNome] = useState("");
  ...
  useEffect(() => {
    if (open) {
      setNome("");
      setCodigo("");
      setFormValidation({ nomeDeficiencia: true, codigo: true });
      setHasChanges(false);
    }
  }, [open]);

  const [formValidation, setFormValidation] = useState({
    nomeDeficiencia: true,
    codigo: true
  });
  ...
  const tratarSubmit = async () => {
    ...
    const missingFields = [];
    if (!nomeDeficiencia.trim()) {
      setFormValidation((prev) => ({ ...prev, nomeDeficiencia: false }));
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Não foi possível cadastrar a deficiência.");
      }
      ...
      if (onDeficienciaCreated) {
        const novaDeficiencia = {
          id: data.data.idDeficiency, 
          nome: nomeDeficiencia,
        };
        onDeficienciaCreated(novaDeficiencia);
      }
    } catch (error) {
      console.error(error.message);
      enqueueSnackbar("Erro ao cadastrar a deficiência.", { variant: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Tooltip title="Cadastre uma deficiência" arrow placement="top">
        <Button
  variant="contained"
  size="small"
  onClick={(event) => {
              ...
              <Grid item xs={12} mb={3}>
                <Stack spacing={1}>
                  ...
                  <TextField
                    ...
                    fullWidth
                    value={nomeDeficiencia}
                    error={
                      !nomeDeficiencia && !formValidation.nomeDeficiencia
                    }
                  />
                </Stack>
              </Grid>
