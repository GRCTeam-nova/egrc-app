  ...
  const handleEditStep = async () => {
    try {
      const getResponse = await axios.get(
        `https://api.egrc.homologacao.com.br/api/v1/action-plans/steps/${row.original.idActionPlanStep}`,
        {
          headers: {
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      const putResponse = await axios.put(
        "https://api.egrc.homologacao.com.br/api/v1/action-plans/steps",
        dadosAtualizados,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
const ListagemAvaliacoes = () => {
  ...
  const handleStepClick = async (stepId) => {
    try {
      const response = await fetch(
        `https://api.egrc.homologacao.com.br/api/v1/action-plans/steps/${stepId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
