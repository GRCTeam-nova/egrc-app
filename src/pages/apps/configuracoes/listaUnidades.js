...
import { useGetUnidade } from "../../../api/unidades";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-unidade`, {
                    state: { indoPara: "NovaUnidade" },
                  });
                }}
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        ...
        const putPayload = {
          ...
          copiarParametro: row.original.copiarParametro,
          segmentos: row.original.segmento,
          ativo: false
        };

        response = await fetch(
          `${API_COMMAND}/api/Unidade/Editar`,
          {
            method: "POST",
            headers: {
      ...
      if (response.ok) {
        setStatus(newStatus);
        if (newStatus === "Inativo") {
          enqueueSnackbar(`Unidade ${row.original.nome} inativada.`, {
            variant: "success",
          ...
          enqueueSnackbar(`Unidade ${row.original.nome} ativada.`, {
            variant: "success",
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: "top",
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a unidade "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa unidade não aparecerá mais no cadastro
            manual.
          </Typography>
        </DialogContent>
        <DialogActions>
...
const ListagemUnidades = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetUnidade(formData, processoSelecionadoId);
      ...
      },
      {
        ...
        accessorFn: (row) => {
          ...
          if (
            ...
            return row.stringUnidadeId.map((segmento) => segmento).join(", ");
          } else {
            return " ";
          }
        },
        cell: ({ row }) => {
          ...
          const stringUnidadeId = row.original.segmento;
          ...
          if (Array.isArray(stringUnidadeId) && stringUnidadeId.length > 0) {
            ...
            return (
              <>
                {stringUnidadeId.map((segmento, index) => (
                  <div key={index}>{segmento.nome}</div>
                ))}
              </>
            );
          } else {
