...
import { useGetTribunal } from "../../../api/tribunais";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/novo-tribunal`, {
                    state: { indoPara: "NovoTribunal" },
                  });
                }}
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        const ufs = row.original.tribunalUF.map((uf) => ({
          id: 0,
          ufId: uf.ufId,
        }));
        const orgaos = row.original.orgaoTribunal
          ? row.original.orgaoTribunal.map((orgao) => ({
              id: 0,
        ...
        response = await fetch(`${API_COMMAND}/api/Tribunal/Editar`, {
          method: "POST",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${API_COMMAND}/api/Tribunal/${row.original.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        enqueueSnackbar(`Tribunal ${row.original.nome} excluído.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir o tribunal: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const TribunalDados = row.original;
              navigation(`/apps/processos/novo-tribunal`, {
                state: {
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o tribunal "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, esse tribunal não aparecerá mais no cadastro manual do
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir o tribunal "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Esse tribunal não será mais disponibilizado ao cadastrar um novo
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Não é possível excluir o Tribunal.
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            O tribunal não pode ser excluído pois está vinculado a processos. É
            possível inativar o tribunal nas configurações de edição.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemTribunal = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetTribunal(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        header: "UF",
        accessorFn: (row) => {
          if (Array.isArray(row.tribunalUF) && row.tribunalUF.length) {
            return row.tribunalUF.map((tribunalUF) => tribunalUF).join(", ");
          } else {
            return " ";
          }
        },
        cell: ({ row }) => {
          const stringUfId = row.original.tribunalUF;

          if (Array.isArray(stringUfId) && stringUfId.length > 0) {
            return (
              <>
                {stringUfId.map((uf, index) => (
                  <div key={index}>{uf.uf?.nome}</div>
                ))}
              </>
            );
          } else {
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshTribunais} />
        ),
      },
    ],
    [theme]
...
export default ListagemTribunal;
