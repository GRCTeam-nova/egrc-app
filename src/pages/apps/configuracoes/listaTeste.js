...
import DrawerTeste from "./novoTesteDrawer";
                                ...
                                >
                                  <Box sx={{ p: 2 }}>
                                    ...
                                    <Button
                                      ...
                                      sx={{ mb: 2 }}
                                      onClick={() =>
                                        onEditPhase({ isNew: true, registro: row.original })
                                      }
                                    >
                                      Nova fase de teste
                                    </Button>
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (response.status === 204) {
        ...
        const message = `Teste ${row.original.name} ${newStatusText.toLowerCase()}.`;
...
const ListagemTestes = () => {
  const theme = useTheme();
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const { acoesJudiciais: lists, isLoading, refetch } = useGetTeste(formData, processoSelecionadoId);
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshOrgaos,
                onEditPhase: handleEditAcionista, // Usa a mesma função para simulação
              }}
            />
          )}
        </Box>
