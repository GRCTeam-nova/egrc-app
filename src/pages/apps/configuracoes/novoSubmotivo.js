...
function ColumnsLayouts() {
  ...
  const [titulo, setTitulo] = useState('Novo Sub-motivo');
...
<Grid container item spacing={4} alignItems="center">
  ...
  <Grid item xs={5}>
    <Stack spacing={1}>
      ...
      <TextField
        onChange={(event) => setNomeArea(event.target.value)}
        fullWidth
        placeholder="Digite o nome do sub-motivo"
        value={nomeArea}
  ...
  {/* Status */}
  <Grid item xs={4} sx={{ marginLeft: '24px' }}> {/* Adicionado espaçamento horizontal */}
    <Stack spacing={1}>
      ...
      {/* Switch e texto Ativo/Inativo alinhados */}
      <Stack direction="row" alignItems="center" spacing={1}>
        <Switch
          checked={status}
...
<Grid item xs={8}>
  <Stack spacing={1}>
    <InputLabel>Atribuir a motivo *</InputLabel>
    <Autocomplete
      multiple
      ...
      renderInput={(params) => (
        <TextField
          {...params}
          placeholder={formData.unidade.length > 0 ? "" : "Selecione o motivo"}
...
<Grid item xs={8}>
  <Stack spacing={1}>
    ...
    <TextField
      ...
      multiline
      rows={4}
      placeholder="Adicione uma descrição para esse sub-motivo"
      value={descricao}
    />
  </Stack>
</Grid>
