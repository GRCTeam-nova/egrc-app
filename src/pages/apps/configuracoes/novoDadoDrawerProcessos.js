...
function ColumnsLayoutsDrawer({ buttonSx, onDataCreated }) {
  ...
  useEffect(() => {
    if (open) {
      setNome("");
      setCodigo("");
      setFormValidation({ nomeDado: true, codigo: true });
      setHasChanges(false);
    }
  }, [open]);
  ...
  const tratarSubmit = async () => {
    ...
    try {
      ...
      enqueueSnackbar(`Dado ${mensagemFeedback} com sucesso!`, {
        variant: "success",
      });
      setOpen(false);
      ...
      if (onDataCreated) {
        ...
        onDataCreated(novoDado);
      }
    } catch (error) {
      console.error(error.message);
      enqueueSnackbar("Erro ao cadastrar o dado.", { variant: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Tooltip title="Cadastre um dado" arrow placement="top">
        <Button
  variant="contained"
  size="small"
  onClick={(event) => {
              ...
              <Grid item xs={12} mb={3}>
                <Stack spacing={1}>
                  ...
                  <TextField
                    ...
                    value={nomeDado}
                    error={
                      !nomeDado && !formValidation.nomeDado
                    }
                  />
                </Stack>
              </Grid>
