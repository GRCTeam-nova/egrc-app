...
import { useGetAssuntoJudicial } from "../../../api/assuntosJudiciais";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/novo-assunto-judicial`, {
                    state: { indoPara: "NovoAssuntoJudicial" },
                  });
                }}
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        // Se o novo status for Inativo, faz a exclusão
        response = await fetch(
          `${API_COMMAND}/api/Assunto/${row.original.id}`,
          {
            method: "DELETE",
            headers: {
        ...
        response = await fetch(
          `${API_COMMAND}/api/Assunto/Editar`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
      ...
      if (response.ok) {
        setStatus(newStatus);
        if (newStatus === "Inativo") {
          enqueueSnackbar(`Assunto judicial ${row.original.nome} inativado.`, {
            variant: "success",
          ...
          enqueueSnackbar(`Assunto judicial ${row.original.nome} ativado.`, {
            variant: "success",
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: "top",
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const AssuntoJudicialDados = row.original;
              navigation(`/apps/processos/novo-assunto-judicial`, {
                state: {
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o assunto "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, esse assunto não aparecerá mais no cadastro
            manual.
          </Typography>
        </DialogContent>
        <DialogActions>
...
const ListagemAssuntosJudiciais = () => {
  const theme = useTheme();
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const {
    assuntosJudiciais: lists,
    isLoading,
    refetch,
  } = useGetAssuntoJudicial(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshAssuntos} />
        ), 
      },
    ],
    [theme]
...
export default ListagemAssuntosJudiciais;
