...
import { useGetAreaJudicial } from "../../../api/areasJudiciais";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-area-judicial`, {
                    state: { indoPara: "NovaAreaJudicial" },
                  });
                }}
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus(newStatus);
        if (newStatus === "Inativo") {
          enqueueSnackbar(`Área judicial ${row.original.nome} inativada.`, {
            variant: "success",
          ...
          enqueueSnackbar(`Área judicial ${row.original.nome} ativada.`, {
            variant: "success",
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: "top",
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const AreaJudicialDados = row.original;
              navigation(`/apps/processos/nova-area-judicial`, {
                state: {
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a área "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa área não aparecerá mais no cadastro
            manual.
          </Typography>
        </DialogContent>
        <DialogActions>
...
const ListagemAreasJudiciais = () => {
  const theme = useTheme();
  const location = useLocation();
  const { processoSelecionadoId } = location.state || {};
  const [formData, setFormData] = useState({ refreshCount: 0 });
  const {
    areasJudiciais: lists,
    isLoading,
    refetch,
  } = useGetAreaJudicial(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshAreas} />
        ),
      },
    ],
    [theme]
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshAreas, // Passa refreshData para ReactTable
              }}
            />
          )}
        </Box>
...
export default ListagemAreasJudiciais;
