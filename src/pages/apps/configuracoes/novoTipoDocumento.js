...
function ColumnsLayouts() {
  ...
  useEffect(() => {
    if (TipoDocumento) {
      ...
      setTitulo(`Editar ${TipoDocumento.nome}`);
      setMensagemFeedback('editada');
      setFormData(prev => ({
        ...prev,
        unidade: TipoDocumento.unidade?.map(u => u.unidadeId) || []
      }));
      setNomeAcao(TipoDocumento.nome);
      setDescricao(TipoDocumento.descricao);
      setStatus(TipoDocumento.ativo || false);
      
    }
  }, [TipoDocumento]);

  useEffect(() => {
    const initialData = TipoDocumento
      ? {
        nomeTipoDocumento: TipoDocumento.nome || '',
        descricao: TipoDocumento.descricao || '',
        status: TipoDocumento.ativo ?? false,
        unidade: TipoDocumento.unidade?.map(u => u.unidadeId) || []
      }
      : {
        nomeTipoDocumento: '',
    ...
    const currentData = {
      nomeTipoDocumento: nomeTipoDocumento ?? (TipoDocumento?.nome || ''),  
      descricao: descricao ?? (TipoDocumento?.descricao || ''),
      status: status ?? (TipoDocumento?.ativo || false),
      unidade: formData.unidade.length || formData.unidade.length === 0
        ? formData.unidade
        : TipoDocumento?.unidade?.map(u => u.unidadeId) || []
    };
    ...
    const hasChanged = initialData.nomeTipoDocumento !== currentData.nomeTipoDocumento ||
  ...
  }, [nomeTipoDocumento, descricao, status, formData.unidade, unidades, TipoDocumento]);
  ...
  const voltarParaCadastroMenu = () => {
    //navigate(-1);
    window.scrollTo(0, 0);
    navigate('/apps/processos/configuracoes-menu', { state: { tab: 'Tipo de Documento' } });
  };
  ...
  const validarForm = () => {
    ...
    const newValidationState = {
      ...
      nomeTipoDocumento: nomeTipoDocumento.trim().length > 0,
    };
  ...
  try {
    if (requisicao !== 'Editar' || (requisicao === 'Editar' && nomeTipoDocumento !== TipoDocumento.nome)) {
      const response = await fetch(`${API_QUERY}/api/TipoDocumento/ExiteNome/${encodeURIComponent(nomeTipoDocumento)}`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nomeExiste) {
          enqueueSnackbar('Essa Tarefa já foi cadastrada com esse nome.', {
            variant: 'error',
            anchorOrigin: {
              vertical: 'top',
              horizontal: 'center',
    ...
    const payload = {
      id: TipoDocumento ? TipoDocumento.id : 0,
      servicoId: "3fa85f64-5717-4562-b3fc-2c963f66afa6",
      unidade: formData.unidade.map((unidade) => {
          return { id: 0, acao: "null", acaoId: TipoDocumento ? TipoDocumento.id : 0, unidadeId: unidade, };
      }),
    ...
    try {
      ...
      if (caracteresEspeciais.test(nomeTipoDocumento)) {
      ...
      const response = await fetch(`${API_COMMAND}/api/TipoDocumento/${requisicao}`, {
  ...
  return (
    <>
        ...
        <Grid item xs={7} sx={{ paddingBottom: 5, marginRight: 5 }}>
          <Stack spacing={1}>
            ...
            <TextField
              ...
              error={!nomeTipoDocumento && formValidation.nomeTipoDocumento === false}
            />
          </Stack>
        </Grid>
      ...
      {requisicao === 'Editar' && (
        <AndamentoHistoricoDrawer row={TipoDocumento} open={drawerOpen} onClose={handleDrawerClose} vindoDe={'Tipo de Andamentos'} />
      )}
    </>
  );
}
