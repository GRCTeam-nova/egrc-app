...
function ColumnsLayouts() {
  ...
  const [titulo, setTitulo] = useState('Novo órgão');
  ...
  useEffect(() => {
    ...
    fetchData(`${API_QUERY}/api/Area/ListarCombo`, setAreas);
    window.scrollTo(0, 0);
  }, []);
  ...
  try {
    if (requisicao !== 'Editar' || (requisicao === 'Editar' && nomeOrgao !== dadosApi.nome)) {
      const response = await fetch(`${API_QUERY}/api/Orgao/ExiteNome/${encodeURIComponent(nomeOrgao)}`, {
        method: 'GET',
        headers: {
    ...
    const payload = {
      id: dadosApi ? dadosApi.id : 0,
      orgaoUnidade: formData.unidade.map((unidade) => {
          return { id: 0,  unidadeId: unidade, };
      }),
      areaOrgao: formData.area.map((area) => {
          return { id: 0, areaId: area, };
      }),
      nome: nomeOrgao,
      descricao: descricao,
      ativo: status,
    ...
    try {
      ...
      const response = await fetch(`${API_COMMAND}/api/Orgao/${requisicao}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
      ...
      if (!response.ok) {
        ...
        try {
          if (response.status === 400) {
            enqueueSnackbar('Não foi possível salvar esse órgão.', {
              variant: 'error',
              anchorOrigin: {
                vertical: 'top',
                horizontal: 'center'
        ...
        enqueueSnackbar(`Órgão ${mensagemFeedback} com sucesso.`, {
          variant: 'success',
          anchorOrigin: {
            vertical: 'top',
            horizontal: 'center'
          ...
          <Stack spacing={1}>
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  placeholder={formData.area.length > 0 ? "" : "Escreva ou selecione uma ou mais áreas"}
        ...
        <Grid item xs={8.5} sx={{ paddingBottom: 5 }}>
          <Stack spacing={1}>
            ...
            <TextField
              ...
              multiline
              rows={4}
              placeholder="Adicione uma descrição para esse órgão"
              value={descricao}
            />
          </Stack>
        </Grid>
