...
function ColumnsLayouts() {
  ...
  useEffect(() => {
    if (TarefaDados) {
      ...
      setTitulo(`Editar ${TarefaDados.nome}`);
      setMensagemFeedback('editada');
      setFormData(prev => ({
        ...prev,
        unidade: TarefaDados.unidade?.map(u => u.unidadeId) || []
      }));
      setNomeAcao(TarefaDados.nome);
      setDescricao(TarefaDados.descricao);
      setStatus(TarefaDados.ativo || false);
      
    }
  }, [TarefaDados]);

  useEffect(() => {
    const initialData = TarefaDados
      ? {
        nomeTarefa: TarefaDados.nome || '',
        descricao: TarefaDados.descricao || '',
        status: TarefaDados.ativo ?? false,
        unidade: TarefaDados.unidade?.map(u => u.unidadeId) || []
      }
      : {
        nomeTarefa: '',
    ...
    const currentData = {
      nomeTarefa: nomeTarefa ?? (TarefaDados?.nome || ''),  
      descricao: descricao ?? (TarefaDados?.descricao || ''),
      status: status ?? (TarefaDados?.ativo || false),
      unidade: formData.unidade.length || formData.unidade.length === 0
        ? formData.unidade
        : TarefaDados?.unidade?.map(u => u.unidadeId) || []
    };
    ...
    const hasChanged = initialData.nomeTarefa !== currentData.nomeTarefa ||
  ...
  }, [nomeTarefa, descricao, status, formData.unidade, unidades, TarefaDados]);
  ...
  const voltarParaCadastroMenu = () => {
    //navigate(-1);
    window.scrollTo(0, 0);
    navigate('/apps/processos/configuracoes-menu', { state: { tab: 'Tarefas' } });
  };
  ...
  const validarForm = () => {
    ...
    const newValidationState = {
      ...
      nomeTarefa: nomeTarefa.trim().length > 0,
    };
  ...
  try {
    if (requisicao !== 'Editar' || (requisicao === 'Editar' && nomeTarefa !== TarefaDados.nome)) {
      const response = await fetch(`${API_QUERY}/api/Tarefa/ExiteNome/${encodeURIComponent(nomeTarefa)}`, {
        method: 'GET',
        headers: {
    ...
    const payload = {
      id: TarefaDados ? TarefaDados.id : 0,
      servicoId: "3fa85f64-5717-4562-b3fc-2c963f66afa6",
      unidade: formData.unidade.map((unidade) => {
          return { id: 0, acao: "null", acaoId: TarefaDados ? TarefaDados.id : 0, unidadeId: unidade, };
      }),
      nome: nomeTarefa,
      descricao: descricao,
      ativo: status,
    ...
    try {
      ...
      if (caracteresEspeciais.test(nomeTarefa)) {
      ...
      const response = await fetch(`${API_COMMAND}/api/Tarefa/${requisicao}`, {
  ...
  return (
    <>
        ...
        <Grid item xs={7} sx={{ paddingBottom: 5, marginRight: 5 }}>
          <Stack spacing={1}>
            ...
            <TextField
              ...
              value={nomeTarefa}
              error={!nomeTarefa && formValidation.nomeTarefa === false}
            />
          </Stack>
        </Grid>
      ...
      {requisicao === 'Editar' && (
        <AndamentoHistoricoDrawer row={TarefaDados} open={drawerOpen} onClose={handleDrawerClose} vindoDe={'Tipo de Andamentos'} />
      )}
    </>
  );
}
