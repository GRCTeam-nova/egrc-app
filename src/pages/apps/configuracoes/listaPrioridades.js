...
import { useGetPrioridade } from "../../../api/prioridades";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-prioridade`, {
                    state: { indoPara: "NovaPrioridade" },
                  });
                }}
                      ...
                      data.length > 0 ? (
                        table.getRowModel().rows.map((row) => (
                          <Fragment key={row.id}>
                            <TableRow
                              style={{
                                borderLeft: `4px solid ${row.original.cor}`,
                              }}
                            >
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const newStatus = status === true ? "Inativa" : "Ativa";
    try {
      let response;
      ...
      if (newStatus === "Inativa") {
        ...
        response = await fetch(`${API_COMMAND}/api/Prioridade/Editar`, {
          method: "POST",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${API_COMMAND}/api/Prioridade/${row.original.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        enqueueSnackbar(`Prioridade ${row.original.nome} excluída.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir a prioridade: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const PrioridadeDados = row.original;
              navigation(`/apps/processos/nova-prioridade`, {
                state: {
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a prioridade "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa prioridade não aparecerá mais no cadastro manual.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir a prioridade "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Essa prioridade não será mais disponibilizada ao cadastrar um novo
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Não é possível excluir a Prioridade.
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            A prioridade não pode ser excluída pois está vinculada a processos.
            É possível inativar a prioridade nas configurações de edição.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemPrioridades = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetPrioridade(formData, processoSelecionadoId);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshPrioridades} />
        ), 
      },
    ],
    [theme]
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshPrioridades,
              }}
            />
          )}
        </Box>
...
export default ListagemPrioridades;
