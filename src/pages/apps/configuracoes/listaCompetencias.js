...
import { useGetCompetencia } from "../../../api/competencias";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const recordType = "Competências";
  const tableRef = useRef(null);
  const [sorting, setSorting] = useState([{ id: "nome", ac: true }]);
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-competencia`, {
                    state: { indoPara: "NovaCompetencia" },
                  });
                }}
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (newStatus === "Inativo") {
        const unidades = row.original.unidade.map((unidade) => ({
          id: 0,
          unidadeId: unidade.unidadeId,
        }));
        const tribunais = row.original.competenciaTribunal
          ? row.original.competenciaTribunal.map((tribunal) => ({
              id: 0,
              tribunalId: tribunal.tribunalId,
            }))
          : [];

        const putPayload = {
          id: row.original.id,
          competenciaUnidade: unidades,
          competenciaTribunal: tribunais,
          nome: row.original.nome,
          descricao: row.original.descricao,
          ativo: false,
        };

        response = await fetch(`${API_COMMAND}/api/Competencia/Editar`, {
          method: "POST",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${API_COMMAND}/api/Competencia/${row.original.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        enqueueSnackbar(`Competência ${row.original.nome} excluída.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir a competência: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        <Stack>
          <Button
            onClick={() => {
              const CompetenciaDados = row.original;
              navigation(`/apps/processos/nova-competencia`, {
                state: {
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a competência "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa competência não aparecerá mais no cadastro manual do
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir a competência "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Essa competência não será mais disponibilizada ao cadastrar um novo
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Não é possível excluir a Competência.
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            A competência não pode ser excluída pois está vinculado a processos. É
            possível inativar o órgão nas configurações de edição.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemCompetencia = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetCompetencia(formData, processoSelecionadoId);
    ...
    () => [
      ...
      },
      {
        ...
        accessorFn: (row) => {
          ...
          if (Array.isArray(row.unidade) && row.unidade.length) {
            return row.unidade.map((unidade) => unidade).join(", ");
          } else {
            return " ";
          }
        },
        cell: ({ row }) => {
          ...
          const stringUnidadeId = row.original.unidade;
            ...
            return (
              <>
                {stringUnidadeId.map((unidade, index) => (
                  <div key={index}>{unidade.nome}</div>
                ))}
              </>
            );
          } else {
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshCompetencias} />
        ), // Passa refreshData
      },
    ],
    [theme]
          ...
          {lists && (
            <ReactTable
              {...{
                ...
                isLoading,
                refreshData: refreshCompetencias,
              }}
            />
          )}
        </Box>
...
export default ListagemCompetencia;
