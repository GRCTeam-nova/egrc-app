...
import { useGetTag } from "../../../api/tags";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/apps/processos/nova-tag`, {
                    state: { indoPara: "NovaTag" },
                  });
                }}
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      if (newStatus === "Inativa") {
        ...
        response = await fetch(`${API_COMMAND}/api/Tag/Editar`, {
          method: "POST",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${API_COMMAND}/api/Tag/${row.original.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        enqueueSnackbar(`Tag ${row.original.nome} excluída.`, {
          variant: "success",
          autoHideDuration: 3000,
          anchorOrigin: {
            vertical: "top",
        ...
        const errorBody = await response.text();
        throw new Error(
          `Falha ao excluir a tag: ${response.status} ${response.statusText} - ${errorBody}`
        );
      }
    } catch (error) {
      console.error("Erro:", error);
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a tag "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa tag não aparecerá mais no cadastro manual
            do processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography
            ...
            style={{ fontWeight: "bold", marginTop: "35px", color: "#717171" }}
          >
            Tem certeza que deseja excluir a tag "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Essa tag não será mais disponibilizada ao cadastrar um novo
            processo.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography
            ...
            style={{ marginTop: "20px", color: "#717171" }}
          >
            A tag não pode ser excluída pois está vinculada a processos.
            É possível inativar a tag nas configurações de edição.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemTags = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetTag(formData, processoSelecionadoId);
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <Chip
            label={row.original.nome}
            sx={{
              backgroundColor: row.original.cor || theme.palette.primary.main, // Use a cor de row.original.cor ou um fallback padrão
      ...
      },
      {
        ...
        disableSortBy: true,
        cell: ({ row }) => (
          <ActionCell row={row} refreshData={refreshTags} />
        ), 
      },
    ],
    [theme]
