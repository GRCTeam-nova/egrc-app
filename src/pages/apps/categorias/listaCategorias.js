...
import { useGetCategorias } from "../../../api/categorias";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const [draftFilters, setDraftFilters] = useState({ categoria: [], status: [], });

  // Abre ou fecha o drawer
  const toggleDrawer = () => setDrawerOpen(!drawerOpen);

  useEffect(() => {
    const categorias = [...new Set(data.map((item) => item.name))];
  ...
  const applyFilters = () => {
    const newFilters = [];
    if (draftFilters.categoria.length > 0) {
      newFilters.push({ type: "Categoria", values: draftFilters.categoria });
    }
  ...
  const removeFilter = (index) => {
    setSelectedFilters((prev) => {
      ...
      setDraftFilters((prevDraft) => {
        ...
        if (filterToRemove.type === "Categoria") {
          updatedDraft.categoria = updatedDraft.categoria.filter(
  ...
  const handleRemoveAllFilters = () => {
    setSelectedFilters([]);
    setGlobalFilter("");
    setDraftFilters({ categoria: [], status: [] });
  };

  // Filtra os categorias com base nos filtros selecionados
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      return selectedFilters.every((filter) => {
        if (filter.type === "Categoria") return filter.values.includes(item.name);
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/categorias/criar`, {
                    state: { indoPara: "NovaCategoria" },
                  });
                }}
            ...
            label={
              <Box display="flex" alignItems="center">
                ...
                <Typography sx={{ color: "#1C5297", fontWeight: 400 }}>
                  {filter.values
                    .map((v) => (v === true ? "Categoria" : v === false ? "Inativo" : v))
                    .join(", ")}
                </Typography>
              </Box>
            }
            ...
            <Grid item xs={12}>
              ...
              <FormControl fullWidth margin="normal">
                <Autocomplete
                  ...
                  onChange={(event, value) => setDraftFilters((prev) => ({ ...prev, categoria: value }))}
                      ...
                      data.length > 0 ? (
                        ...
                        <TableRow>
                          <TableCell colSpan={columns.length}>
                            <EmptyTable msg="Categorias não encontrados" />
                          </TableCell>
                        </TableRow>
                      )
                    ) : (
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const idCategory = row.original.idCategory;
    const newStatus = status === true ? "Inativo" : "Ativo";
    
    try {
      // Buscar os dados do departamento pelo ID
      const getResponse = await axios.get(`https://api.egrc.homologacao.com.br/api/v1/categories/${idCategory}`, {
        headers: {
      ...
      await axios.put("https://api.egrc.homologacao.com.br/api/v1/categories", dadosAtualizados, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setStatus(newStatus);
      const message = `Categoria ${row.original.name} ${newStatus.toLowerCase()}.`;
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar a categoria "{row.original.name}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, essa categoria não aparecerá mais no cadastro manual.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemEmpresa = () => {
  ...
  const {
    ...
    isLoading,
  } = useGetCategorias(formData, processoSelecionadoId);
