...
import { useGetObjetivos } from "../../../api/objetivos";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const recordType = "Objetivos de controle";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/objetivos/criar`, {
                    state: { indoPara: "NovoObjetivo" },
                  });
                }}
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const idObjective = row.original.idObjective;
    const newStatus = status === true ? "Inativo" : "Ativo";
    
    try {
      // Buscar os dados do departamento pelo ID
      const getResponse = await axios.get(`https://api.egrc.homologacao.com.br/api/v1/objective/${idObjective}`, {
        headers: {
      ...
      await axios.put("https://api.egrc.homologacao.com.br/api/v1/objective", dadosAtualizados, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setStatus(newStatus);
      const message = `Objetivo ${row.original.name} ${newStatus.toLowerCase()}.`;
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o objetivo "{row.original.name}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, esse objetivo não aparecerá mais no cadastro manual.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemEmpresa = () => {
  ...
  const {
    ...
    isLoading,
  } = useGetObjetivos(formData, processoSelecionadoId);
