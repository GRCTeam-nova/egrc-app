...
import { useGetIpes } from "../../../api/ipes";
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/ipes/criar`, {
                    state: { indoPara: "NovoIpe" },
                  });
                }}
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const idInformationActivity = row.original.idInformationActivity;
    const newStatus = status === true ? "Inativo" : "Ativo";
    
    try {
      // Buscar os dados do departamento pelo ID
      const getResponse = await axios.get(`https://api.egrc.homologacao.com.br/api/v1/ipe/${idInformationActivity}`, {
        headers: {
      ...
      await axios.put("https://api.egrc.homologacao.com.br/api/v1/ipe", dadosAtualizados, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setStatus(newStatus);
      const message = `IPE ${row.original.name} ${newStatus.toLowerCase()}.`;
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o IPE "{row.original.name}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, esse IPE não aparecerá mais no cadastro manual.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemEmpresa = () => {
  ...
  const {
    ...
    isLoading,
  } = useGetIpes(formData, processoSelecionadoId);
