...
import { useGetCiclos } from "../../../api/ciclos";
...
function ReactTable({ data, columns, processosTotal, isLoading }) {
  ...
  const [draftFilters, setDraftFilters] = useState({ ciclo: [], status: [], });

  // Abre ou fecha o drawer
  const toggleDrawer = () => setDrawerOpen(!drawerOpen);

  useEffect(() => {
    const ciclos = [...new Set(data.map((item) => item.name))];
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      return selectedFilters.every((filter) => {
        if (filter.type === "Ciclo") return filter.values.includes(item.name);
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  navigation(`/ciclos/criar`, {
                    state: { indoPara: "NovoCiclo" },
                  });
                }}
            ...
            label={
              <Box display="flex" alignItems="center">
                ...
                <Typography sx={{ color: "#1C5297", fontWeight: 400 }}>
                  {filter.values
                    .map((v) => (v === true ? "Ciclo" : v === false ? "Inativo" : v))
                    .join(", ")}
                </Typography>
              </Box>
            }
            ...
            <Grid item xs={12}>
              ...
              <FormControl fullWidth margin="normal">
                <Autocomplete
                  ...
                  onChange={(event, value) => setDraftFilters((prev) => ({ ...prev, ciclo: value }))}
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const idCycle = row.original.idCycle;
    const newStatus = status === true ? "Inativo" : "Ativo";
    
    try {
      // Buscar os dados do departamento pelo ID
      const getResponse = await axios.get(`https://api.egrc.homologacao.com.br/api/v1/cycles/${idCycle}`, {
        headers: {
      ...
      await axios.put("https://api.egrc.homologacao.com.br/api/v1/cycles", dadosAtualizados, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setStatus(newStatus);
      const message = `Ciclo ${row.original.name} ${newStatus.toLowerCase()}.`;
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o ciclo "{row.original.name}"?
          </Typography>
          <Typography
            component="div"
            style={{ marginTop: "20px", color: "#717171" }}
          >
            Ao inativar, esse ciclo não aparecerá mais no cadastro manual.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
...
const ListagemEmpresa = () => {
  ...
  const {
    ...
    isLoading,
  } = useGetCiclos(formData, processoSelecionadoId);
