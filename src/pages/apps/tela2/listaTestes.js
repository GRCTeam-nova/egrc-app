...
import { useGetTeste } from "../../../api/testes";
...
function ReactTable({
  ...
  const filteredData = useMemo(() => {
    ...
    // 2) se não houver busca de texto, retorna já filtrado
    if (!globalFilter) return base;

    // 3) normalização (sem acento, lowercase)
    ...
    // 4) filtra no campo "name" (ajuste se quiser outro campo)
    return base.filter((item) => normalize(item.name || "").includes(term));
  }, [data, isActiveFilter, isInactiveFilter, globalFilter]);
          ...
          <Autocomplete
            ...
            onChange={(event, value) => {
              ...
              if (typeof onProjectChange === "function") {
                onProjectChange(value ? value.idProject : null);
              }
            }}
            renderInput={(params) => (
              <TextField
                ...
                size="small"
                sx={{
                  width: "300px", // Diminuindo a largura
                      ...
                      <InputAdornment position="start">
                        <FolderOpenOutlinedIcon
                          style={{
                            ...
                            marginTop: -2,
                            color: "#254C7D",
                            fontSize: "16px", // tamanho reduzido
                          }}
                        />
                      </InputAdornment>
                    ...
                    {!selectedProject ? (
                      <TableRow>
                        <TableCell colSpan={columns.length}>
                          <EmptyTable msg="Selecione um projeto" />
                        </TableCell>
                      </TableRow>
                    ) : isLoading ? (
                      <TableRow>
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const idTest = row.original.idTest;
    ...
    try {
      ...
      setStatus(newStatus);
      const message = `Teste ${row.original.name} ${newStatus.toLowerCase()}.`;
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Tem certeza que deseja inativar o teste "{row.original.nome}"?
          </Typography>
          <Typography
            component="div"
...
const ListagemEmpresa = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetTeste(formData, processoSelecionadoId);
        ...
        <Box>
          {lists && (
            <ReactTable
              data={lists}
              columns={columns}
              processosTotal={processosTotal}
              isLoading={isLoading}
              refreshData={refreshOrgaos}
              onProjectChange={(idProject) => {
                setFormData((currentData) => ({
                  ...currentData,
                  projectId: idProject,
                  refreshCount: currentData.refreshCount + 1, // força o refresh da consulta
                }));
              }}
            />
          )}
