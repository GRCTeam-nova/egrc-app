...
function ReactTable({ data, columns, processosTotal, isLoading, refreshData }) {
  ...
  const [sorting, setSorting] = useState([{ id: "dataSourceName", asc: true }]);
  ...
  return (
    <>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center" justifyContent="space-between" sx={{ backgroundColor: "#F1F1F1E5", p: 2, mb: 3 }}>
        <Stack direction="row" spacing={1} alignItems="center">
          <DebouncedInput
            value={globalFilter ?? ""}
            onFilterChange={(value) => setGlobalFilter(String(value))}
            placeholder={`Pesquise pelo nome`}
            style={{ width: "350px", height: "33px", borderRadius: "8px", border: "0.3px solid #00000010", backgroundColor: "#FFFFFF" }}
          />
          <Button
            onClick={toggleDrawer}
            startIcon={<FontAwesomeIcon icon={faFilter} style={{ color: "#00000080" }} />}
            style={{ width: "90px", color: "#00000080", backgroundColor: 'white', fontSize: "13px", marginLeft: 24, fontWeight: 400, height: "33px", borderRadius: "8px", border: "0.6px solid #00000040 " }}
          >
            Filtros
          </Button>
        </Stack>

        <Button variant="contained" onClick={() => navigation(`/fonteDeDados/criar`)} startIcon={<PlusOutlined />} style={{ borderRadius: "20px", height: "32px" }}>Novo</Button>
      </Stack>

      <Box mb={2}>
        {selectedFilters.map((filter, index) => (
          <Chip key={index} label={`${filter.type}: ${filter.values.map(v => v ? 'Ativo' : 'Inativo').join(', ')}`} onDelete={() => removeFilter(index)} sx={{ margin: 0.5, backgroundColor: "#1C52971A" }} />
        ))}
        {selectedFilters.length > 0 && <Button onClick={handleRemoveAllFilters} sx={{ fontSize: '12px' }}>Limpar Filtros</Button>}
      </Box>
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    try {
      const payload = {
        id: row.original.id,
        dataSourceCode: row.original.dataSourceCode,
        dataSourceName: row.original.dataSourceName,
        active: !row.original.active
      };
      await axios.put(`https://api.egrc.homologacao.com.br/api/v1/DataSource`, payload, {
...
const ListagemFonteDeDados = () => {
  ...
  const columns = useMemo(() => [
    { header: "Código", accessorKey: "dataSourceCode" },
    {
      header: "Nome da Fonte de Dados",
      accessorKey: "dataSourceName",
      cell: ({ row }) => (
        <Typography onClick={() => navigation(`/fonteDeDados/editar/${row.original.dataSourceCode || row.original.id}`, { state: { fonteDeDadosDados: row.original } })} sx={{ cursor: "pointer", color: "#1C5297", fontWeight: 500 }}>
          {row.original.dataSourceName}
        </Typography>
      ),
    },
    { header: "Status", accessorKey: "active", cell: ({ row }) => <Chip label={row.original.active ? "Ativo" : "Inativo"} color={row.original.active ? "success" : "default"} variant="outlined" size="small" /> },
    { header: "Ações", cell: ({ row }) => <ActionCell row={row} refreshData={fetchData} /> },
  ], []);

  return (
    <Box p={3}>
      <Typography variant="h4" sx={{ mb: 2, color: '#1C5297', fontWeight: 600 }}>Fontes de Dados</Typography>
      <ReactTable data={data} columns={columns} processosTotal={data.length} isLoading={isLoading} refreshData={fetchData} />
    </Box>
  );
};

export default ListagemFonteDeDados;
