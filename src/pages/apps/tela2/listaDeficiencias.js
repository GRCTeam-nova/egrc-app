...
import { useGetDeficiencias } from "../../../api/deficiencias";
...
function ActionCell({ row, refreshData }) {
  ...
  const toggleStatus = async () => {
    const idDeficiency  = row.original.idDeficiency ;
    const newStatus = status === true ? "Inativo" : "Ativo";
    
    try {
      // Buscar os dados do departamento pelo ID
      const getResponse = await axios.get(`${API_URL}deficiencies/${idDeficiency}`, {
        headers: {
      ...
      await axios.put(`${API_URL}deficiencies`, dadosAtualizados, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setStatus(newStatus);
      const message = `Deficiência ${row.original.name} ${newStatus.toLowerCase()}.`;
...
const ListagemEmpresa = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetDeficiencias(formData, processoSelecionadoId);
