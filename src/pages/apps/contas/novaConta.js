...
function ColumnsLayouts() {
  ...
  useEffect(() => {
    fetchData(`${process.env.REACT_APP_API_URL}companies`, setEmpresas);
    fetchData(`${process.env.REACT_APP_API_URL}processes`, setProcessos);
    fetchData(`${process.env.REACT_APP_API_URL}ledger-accounts/types`, setTiposResponsabilidades);
    fetchData(`${process.env.REACT_APP_API_URL}ledger-accounts`, setContas);
    fetchData(`${process.env.REACT_APP_API_URL}ledger-accounts/assertions`, setAssertions);
    fetchData(`${process.env.REACT_APP_API_URL}ledger-accounts`, setContasSuperiores);
  ...
  const handleProcessCreated = (newProcesso) => {
    // Adiciona o novo processo apenas na lista de opções (Autocomplete)
    // NÃO seleciona automaticamente para não conflitar com os filtros ativos.
  ...
  // --- EFEITO: EMPRESA FILTRA PROCESSO ---
  useEffect(() => {
    const atualizarProcessos = async () => {
      // Se não há empresas selecionadas, mostra todos os processos
      ...
      const idsProcessoPermitidos = new Set();
      const novoMapaProcesso = {};
      ...
      try {
        ...
        results.forEach((res) => {
          const nomeEmpresa = res.data.name;
          const procs = res.data.idProcesses || res.data.processes || [];

          procs.forEach((p) => {
            const idProc = typeof p === "object" ? p.idProcess || p.id : p;
            if (idProc) {
              idsProcessoPermitidos.add(idProc);
              if (!novoMapaProcesso[idProc]) novoMapaProcesso[idProc] = [];
              if (!novoMapaProcesso[idProc].includes(nomeEmpresa)) {
                novoMapaProcesso[idProc].push(nomeEmpresa);
              }
            }
          });
        });

        // Função de segurança rigorosa para identificar processos órfãos
        ...
        setProcessosFiltrados(
          processos.filter(
            (p) => idsProcessoPermitidos.has(p.id) || isOrphan(p, ["companies", "idCompanies"])
          )
        );
        setProcessoOrigemMap(novoMapaProcesso);
      } catch (e) {
        console.error("Erro ao buscar dependências da empresa:", e);
      }
    };
  ...
  }, [formData.empresa, processos, token]);

  const handleSelectEmpresa = (event, newValue) => {
    let novosIds = [];
    if (newValue.length > 0 && newValue[newValue.length - 1].id === "all") {
      if (formData.empresa.length === empresas.length) novosIds = [];
      else novosIds = empresas.map((e) => e.id);
    } else {
      novosIds = newValue.map((item) => item.id);
    }

    // Interceptação: Se limpou empresas (ou alterou a origem) e há processos selecionados
    if (formData.empresa.length === 0 && novosIds.length > 0 && formData.processo.length > 0) {
      setPendingAction({ type: "empresa", ids: novosIds });
      setWarningDialogOpen(true);
      return;
    }
    tratarMudancaInputGeral("empresa", novosIds);
  };

  const confirmarMudancaFiltro = () => {
    if (pendingAction.type === "empresa") {
      setFormData((prev) => ({
        ...prev,
        empresa: pendingAction.ids,
        processo: [], // Limpa dependentes incompatíveis
      }));
    }
  ...
  const handleSelectAll2 = (event, newValue) => {
    if (newValue.length > 0 && newValue[newValue.length - 1].id === "all") {
      if (formData.processo.length === processosFiltrados.length) {
        ...
        setFormData({ ...formData, processo: processosFiltrados.map((p) => p.id) });
      }
    } else {
  ...
  const handleSelectAllConta = (event, newValue) => {
    if (newValue.length > 0 && newValue[newValue.length - 1].id === "all") {
      ...
      if (formData.conta.length === filteredContas.length) {
        ...
        setFormData({ ...formData, conta: filteredContas.map((conta) => conta.id) });
      }
    } else {
      tratarMudancaInputGeral("conta", newValue.map((item) => item.id));
    }
  };
  ...
  const handleSelectAllAssertion = (event, newValue) => {
    if (newValue.length > 0 && newValue[newValue.length - 1].id === "all") {
      if (formData.assertion.length === assertions.length) {
        ...
        setFormData({ ...formData, assertion: assertions.map((assertion) => assertion.id) });
      }
    } else {
      tratarMudancaInputGeral("assertion", newValue.map((item) => item.id));
    }
  };
  ...
  useEffect(() => {
    ...
    if (superiorSelecionada && formatarNome(superiorSelecionada.nome) === nomeDigitado) {
      setFormData((prev) => ({ ...prev, contaSuperior: null }));
    }
    ...
    if (inferioresAtualizadas.length !== formData.conta.length) {
      setFormData((prev) => ({ ...prev, conta: inferioresAtualizadas }));
    }
  }, [nomeConta, contasSuperiores, contas, formData.contaSuperior, formData.conta]);
  ...
  const allSelected = formData.empresa.length === empresas.length && empresas.length > 0;
  const allSelected2 = formData.processo.length === processosFiltrados.length && processosFiltrados.length > 0;
  const allSelectedContas = formData.conta.length === contas.length && contas.length > 0;
  const allSelectedAssertions = formData.assertion.length === assertions.length && assertions.length > 0;
  ...
  const tratarSubmit = async () => {
    ...
    if (requisicao === "Criar") {
      ...
      payload = {
        ...
        idLedgerAccountType: formData.tipoConta,
        idLedgerAccountSuperior: formData.contaSuperior === "" ? null : formData.contaSuperior,
        ...
        <Grid container spacing={1} marginTop={2}>
          ...
          <Grid item xs={6} sx={{ paddingBottom: 5 }}>
            <Stack spacing={1}>
              ...
              <Autocomplete
                ...
                value={
                  tiposResponsabilidades.find((tipoConta) => tipoConta.id === formData.tipoConta) || null
                }
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    error={!formData.tipoConta && formValidation.tipoConta === false}
                  />
                )}
              />
            </Stack>
          ...
          {requisicao === "Editar" && (
            <>
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        error={
                          (formData.empresa.length === 0 || formData.empresa.every((val) => val === 0)) &&
              ...
              <Grid item xs={6} sx={{ paddingBottom: 5 }}>
                <Stack spacing={1}>
                  ...
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option.nome || ""}
                    value={
                      contasSuperiores.find((contaSuperior) => contaSuperior.id === formData.contaSuperior) || null
                    }
                    onChange={(event, newValue) => {
                      setFormData((prev) => ({
                        ...prev,
                    ...
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        error={!formData.contaSuperior && formValidation.contaSuperior === false}
                      />
                    )}
                  />
                </Stack>
              ...
              {/* AUTOCOMPLETE: CONTAS INFERIORES */}
              <Grid item xs={6} sx={{ paddingBottom: 5 }}>
                <Stack spacing={1}>
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        placeholder={formData.conta.length > 0 ? "" : "Escreva ou selecione uma ou mais contas"}
                        error={
                          (formData.conta.length === 0 || formData.conta.every((val) => val === 0)) &&
              ...
              <Grid item xs={6} sx={{ paddingBottom: 5 }}>
                <Stack spacing={1}>
                  ...
                  <Autocomplete
                    multiple
                    disableCloseOnSelect
                    options={[{ id: "all", nome: "Selecionar todas" }, ...assertions]}
                    getOptionLabel={(option) => option.nome || ""}
                    ...
                    renderOption={(props, option, { selected }) => (
                      <li {...props}>
                        <Grid container alignItems="center">
                          <Grid item>
                            <Checkbox checked={option.id === "all" ? allSelectedAssertions : selected} />
                          </Grid>
                          <Grid item xs>
                            {option.nome}
                          </Grid>
                    ...
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        error={
                          (formData.assertion.length === 0 || formData.assertion.every((val) => val === 0)) &&
              ...
              <Grid item xs={6} mb={5}>
                <Stack spacing={1}>
                  ...
                  <Autocomplete
                    multiple
                    disableCloseOnSelect
                    options={processosFiltrados.length > 0 ? [{ id: "all", nome: "Selecionar todas" }, ...processosFiltrados] : []}
                    ...
                    isOptionEqualToValue={(option, value) => option.id === value.id}
                    renderOption={(props, option, { selected }) => {
                      const origens = processoOrigemMap[option.id] ? processoOrigemMap[option.id].join(", ") : "";
                      return (
                        <li {...props}>
                          <Grid container alignItems="center">
                            <Grid item>
                              <Checkbox checked={option.id === "all" ? allSelected2 : selected} />
                            </Grid>
                            <Grid item xs>
              ...
              <Grid item xs={2} mb={2}>
                <Stack spacing={1}>
                  <Stack direction="row" alignItems="center" spacing={1} style={{ marginTop: 0.5 }}>
                    <Switch checked={relevante} onChange={(event) => setRelevante(event.target.checked)} />
          ...
          >
            ...
            <DialogContent>
              <DialogContentText sx={{ fontSize: "16px", color: "#555", px: 2 }}>
                A conta foi cadastrada com sucesso. Você pode voltar para a listagem ou adicionar mais informações a essa conta.
              </DialogContentText>
            </DialogContent>
          ...
          <Dialog open={warningDialogOpen} onClose={() => setWarningDialogOpen(false)}>
            ...
            <DialogContent>
              <DialogContentText>
                Ao alterar a seleção de Empresas, a lista de Processos será filtrada e exibirá apenas correspondências válidas.
                <br /><br />
                <strong>Os Processos vinculados que não corresponderem à nova seleção serão removidos.</strong>
                <br /><br />
                Deseja continuar?
              </DialogContentText>
            </DialogContent>
