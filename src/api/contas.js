...
export function useGetContas(formData = {}) {
  ...
  useEffect(() => {
    ...
    let url = "https://api.egrc.homologacao.com.br/api/v1/ledger-accounts/reports";
    const params = [];
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Erro ao buscar os dados de riscos");
        }
        ...
        if (GenerateExcel === true) {
          ...
          setCustomers(data.reportLedgerAccounts);
        }
      } catch (err) {
